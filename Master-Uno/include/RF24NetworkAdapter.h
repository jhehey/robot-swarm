...
char readBuffer[MAX_PAYLOAD_SIZE];					// Default payload size is 144 bytes (RF24Network_config.h)
...
bool SendMessage(uint8_t to, const char* message)
{
	...
	bool ok = network.multicast(header, &dataBuffer, MAX_PAYLOAD_SIZE, 1);
	if(!ok) {
...
void ReceiveMessage(RF24NetworkHeader& header, uint16_t& payloadSize)
{
	network.read(header, &readBuffer, payloadSize);
	...
	receiveMessageCallback(readBuffer);
